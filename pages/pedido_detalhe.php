...
if (!$pedido_id) {
    ...
    // Busca detalhes do pedido (sem reputacao)
    ...
    if ($result_pedido && pg_num_rows($result_pedido) === 1) {
        ...
        $page_title = htmlspecialchars($pedido['titulo']); 

        // === CORREÇÃO: Remove u.reputacao da query ===
        $sql_comentarios = "SELECT c.*, u.nome as autor_nome, u.id as autor_id 
                            FROM comentarios c 
                            JOIN usuarios u ON c.usuario_id = u.id 
                            WHERE c.pedido_id = $1 
                            ORDER BY c.data_comentario ASC";
                            
        if (!@pg_prepare($conn, "get_comentarios", $sql_comentarios)) { die("Erro ao preparar consulta de comentários."); } // Este era o erro
        $result_comentarios = pg_execute($conn, "get_comentarios", array($pedido_id));
        
        if ($result_comentarios) {
            // Organiza comentários em árvore (sem alterações)
...
// === CORREÇÃO: Função atualizada para usar link de perfil (remove reputacao) ===
function renderizar_comentarios($comentarios_nivel, $nivel = 0) {
    ...
    foreach ($comentarios_nivel as $comentario) {
        ...
        // Link para o novo perfil público
        echo "<a href='".BASE_URL."/pages/usuario_perfil.php?id={$autor_id}' class='comentario-autor fw-bold text-decoration-none'>".htmlspecialchars($comentario['autor_nome'])."</a>";
...
<main class="container my-5">
    ...
    ?>
        <div class="detalhe-container">
            ...
            <div class="pedido-autor mb-4 pb-3 border-bottom border-light">
                <img src="https://api.dicebear.com/8.x/initials/svg?seed=<?php echo $avatar_seed; ?>" alt="Avatar" class="autor-avatar">
                <div>
                    <a href="<?php echo BASE_URL; ?>/pages/usuario_perfil.php?id=<?php echo $pedido['usuario_id']; ?>" class="autor-nome d-block text-decoration-none"><?php echo htmlspecialchars($pedido['autor_nome']); ?></a>
            ...
            <div class="detalhe-meta d-flex flex-wrap align-items-center gap-4 mb-4 pb-4 border-bottom border-light">
            ...
            <div class="detalhe-footer mt-4 mb-5 text-center">
                <?php if ($e_dono): ?>
                    <a href="<?php echo BASE_URL; ?>/pages/caixa_entrada.php" class="btn btn-primary btn-lg ajudar-btn"> <i data-lucide="message-square"></i> Ver Conversas Deste Pedido </a>
                <?php elseif (isset($_SESSION['usuario_id'])): ?>
                    <form action="<?php echo BASE_URL; ?>/includes/iniciar_chat.php" method="POST"> <input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>"> <input type="hidden" name="pedido_id" value="<?php echo $pedido['id']; ?>"> <button type="submit" class="btn btn-success btn-lg ajudar-btn"> <i data-lucide="message-circle"></i> Iniciar Conversa com <?php echo htmlspecialchars(explode(' ', $pedido['autor_nome'])[0]); ?> </button> </form>
                <?php else: ?>
                    <a href="<?php echo BASE_URL; ?>/pages/login.php" class="btn btn-success btn-lg ajudar-btn"> <i data-lucide="log-in"></i> Faça login para conversar com <?php echo htmlspecialchars(explode(' ', $pedido['autor_nome'])[0]); ?> </a>
                <?php endif; ?>
            </div>

            <div class="comentarios-section pt-4">
                <h3 class="mb-4 text-center">Discussão Pública (<?php echo count($comentarios); ?>)</h3>
                <?php if (isset($_SESSION['usuario_id'])): ?>
                <div class="card p-3 mb-4 bg-light border-0">
                    <form action="../includes/processa_comentario.php" method="POST" class="form-comentario">
                        <input type='hidden' name='csrf_token' value='<?php echo $_SESSION['csrf_token']; ?>'>
                        <input type='hidden' name='pedido_id' value='<?php echo $pedido['id']; ?>'>
                        <div class="mb-3"><textarea class="form-control" name="comentario" rows="3" placeholder="Deixe sua pergunta ou mensagem de apoio pública..." required></textarea></div>
                        <div class="text-end"><button type="submit" class="btn btn-success"><i data-lucide="send"></i> Enviar Comentário</button></div>
                    </form>
                </div>
                <?php else: ?>
                <div class="alert alert-light text-center"> <a href="<?php echo BASE_URL; ?>/pages/login.php">Faça login</a> para participar da discussão pública. </div>
                <?php endif; ?>
