...
$include_chartjs = true; // Habilita o script do Chart.js no header

// 1. Ordem correta de inclusão
...
// =======================================================
// LÓGICA DE BUSCA ESTATÍSTICAS (GRÁFICOS)
// =======================================================
...
if ($result_cat) { while($row = pg_fetch_assoc($result_cat)) { $dados_categoria['labels'][] = $row['categoria']; $dados_categoria['data'][] = (int)$row['total']; } }
// Dados Gráfico Urgência
...
if ($result_urg) { while($row = pg_fetch_assoc($result_urg)) { $dados_urgencia['labels'][] = $row['urgencia']; $dados_urgencia['data'][] = (int)$row['total']; } }


// =======================================================
// LÓGICA DE FILTRO E PAGINAÇÃO (MEUS PEDIDOS)
// =======================================================
...
// =======================================================
// LÓGICA PARA SEÇÕES DO VOLUNTÁRIO
// =======================================================
...
<main class="container my-5">
    ...
    <div class="row mb-5 g-4">
        <div class="col-lg-3 col-md-6"><div class="stat-card h-100"><div class="stat-card-icon" style="background-color: #3B82F6;"><i data-lucide="list"></i></div><h5 class="stat-card-title">Total de Pedidos</h5><p class="stat-card-number"><?php echo $total; ?></p></div></div>
        <div class="col-lg-3 col-md-6"><div class="stat-card h-100"><div class="stat-card-icon" style="background-color: var(--danger-color);"><i data-lucide="siren"></i></div><h5 class="stat-card-title">Urgentes</h5><p class="stat-card-number"><?php echo $urgentes; ?></p></div></div>
        <div class="col-lg-3 col-md-6"><div class="stat-card h-100"><div class="stat-card-icon" style="background-color: var(--warning-color);"><i data-lucide="clock"></i></div><h5 class="stat-card-title">Abertos</h5><p class="stat-card-number"><?php echo $abertos; ?></p></div></div>
        <div class="col-lg-3 col-md-6"><div class="stat-card h-100"><div class="stat-card-icon" style="background-color: var(--primary-color);"><i data-lucide="check-circle-2"></i></div><h5 class="stat-card-title">Concluídos</h5><p class="stat-card-number"><?php echo $concluidos; ?></p></div></div>
    </div>
    ...
    <div id="lista-pedidos-dash" class="row">
        ...
        if ($result_all && pg_num_rows($result_all) > 0) {
            while($row = pg_fetch_assoc($result_all)) {
                ...
                echo "<div class='col-lg-12' id='pedido-dash-".$row['id']."'>";
                ...
                echo "      </div>";
                echo "      <a href='pedido_detalhe.php?id=" . $row['id'] . "' class='pedido-link-titulo'><h3>" . htmlspecialchars($row['titulo']) . "</h3></a>";
                echo "      <div class='pedido-meta border-top pt-3 mt-3 d-flex justify-content-between align-items-center'>";
                echo "          <span><i data-lucide='message-square'></i> " . $row['total_comentarios'] . " Comentários</span>";
                echo "          <div class='d-flex gap-2'>";
                if ($row['status'] == 'Aberto') {
                    echo "      <button class='btn btn-sm btn-success' onclick='marcarConcluido(".$row['id'].")'><i data-lucide='check-circle'></i> Concluir</button>";
                }
                echo "          <a href='editar_pedido.php?id=".$row['id']."' class='btn btn-sm btn-outline-primary'><i data-lucide='edit-2'></i> Editar</a>";
                echo "          <form action='../includes/excluir_pedido.php' method='POST' onsubmit='return confirm(\"Tem certeza?\");' style='display:inline;'>";
                echo "              <input type='hidden' name='csrf_token' value='".$_SESSION['csrf_token']."'>";
                echo "              <input type='hidden' name='pedido_id' value='".$row['id']."'>";
                echo "              <button type='submit' class='btn btn-sm btn-outline-danger'><i data-lucide='trash-2'></i> Excluir</button>";
                echo "          </form>";
                echo "          </div>";
                echo "      </div>";
                echo "  </div>";
        ...
        } elseif ($total > 0 && $total_pedidos_filtrados == 0) {
            echo "<div class='col-12 text-center card p-5'><p class='lead'>Nenhum pedido encontrado com os filtros aplicados.</p><a href='dashboard.php' class='btn btn-success mt-3 mx-auto' style='width: auto;'>Limpar Filtros</a></div>";
        } else {
            echo "<div class='col-12 text-center card p-5'><p class='lead'>Você ainda não cadastrou nenhum pedido.</p><a href='cadastrar.php' class='btn btn-success mt-3 mx-auto' style='width: auto;'><i data-lucide='plus-circle'></i> Criar meu primeiro pedido</a></div>";
        }
        ?>
    </div>
    ...
    <nav aria-label="Paginação Dashboard" class="mt-5">
        <ul class="pagination justify-content-center">
            <li class="page-item <?php echo ($page <= 1) ? 'disabled' : ''; ?>"><a class="page-link" href="<?php echo construir_url_paginacao($page - 1, $busca, $status_filtro); ?>">Anterior</a></li>
            <?php for ($i = 1; $i <= $total_paginas; $i++): ?>
                <li class="page-item <?php echo ($i == $page) ? 'active' : ''; ?>"><a class="page-link" href="<?php echo construir_url_paginacao($i, $busca, $status_filtro); ?>"><?php echo $i; ?></a></li>
            <?php endfor; ?>
             <li class="page-item <?php echo ($page >= $total_paginas) ? 'disabled' : ''; ?>"><a class="page-link" href="<?php echo construir_url_paginacao($page + 1, $busca, $status_filtro); ?>">Próxima</a></li>
        </ul>
    </nav>
    <?php endif; ?>
    ...
    <div class="row g-5">
            ...
            <div class="card p-3" style="border-radius: var(--radius-lg); box-shadow: var(--shadow-sm);">
                ...
                <?php else: ?>
                    <div class="list-group list-group-flush">
                        <?php foreach ($conversas_iniciadas as $conversa): $avatar_seed_criador = urlencode($conversa['criador_nome']); ?>
        ...
        <div class="col-md-6">
            ...
            <div class="card p-3" style="border-radius: var(--radius-lg); box-shadow: var(--shadow-sm);">
                ...
                <?php else: ?>
                    <div class="list-group list-group-flush">
                        <?php foreach ($pedidos_comentados as $pedido_com): $avatar_seed_autor = urlencode($pedido_com['autor_nome']); ?>
