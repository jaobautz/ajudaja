...
include_once '../includes/session.php'; // Incluído para garantir o token

$usuario_id = $_SESSION['usuario_id'];

// ... (toda a sua lógica de busca de dados do dashboard permanece a mesma) ...
...
<main class="container my-5">
    ...
    <?php if ($total > 0): ?>
    <div class="row mb-5 g-4">
        <div class="col-md-6"><div class="card h-100" style="border-radius: var(--radius-lg); box-shadow: var(--shadow-sm); border: 1px solid var(--border-color);"><div class="card-body"><h5 class="card-title text-center mb-3">Pedidos por Categoria</h5><canvas id="grafico-categoria"></canvas></div></div></div>
        <div class="col-md-6"><div class="card h-100" style="border-radius: var(--radius-lg); box-shadow: var(--shadow-sm); border: 1px solid var(--border-color);"><div class="card-body"><h5 class="card-title text-center mb-3">Pedidos por Urgência</h5><canvas id="grafico-urgencia"></canvas></div></div></div>
    </div>
    <?php endif; ?>
    ...
    <div id="lista-pedidos-dash" class="row">
        ...
        if ($result_all && pg_num_rows($result_all) > 0) {
            while($row = pg_fetch_assoc($result_all)) {
                ...
                echo "<div class='col-lg-12' id='pedido-dash-".$row['id']."'>"; // ID para o JS
                ...
                // --- TAREFA 2: Botões de Ação Seguros ---
                ...
                echo "          <form action='../includes/excluir_pedido.php' method='POST' onsubmit='return confirm(\"Tem certeza que quer excluir este pedido? Esta ação é irreversível.\");' style='display:inline;'>";
